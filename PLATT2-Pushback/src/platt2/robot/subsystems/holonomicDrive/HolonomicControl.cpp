...
void HolonomicControl::moveToPoint(double x_target, double y_target, double target_heading, double rSpeed, double wSpeed, double timeout) {
    ...
    while (true){
        ...
        motionVector.r = std::clamp(-1*(positionPID->calculate(0, p.r)), -rSpeed, rSpeed); //neeed to add motion curve 
...
double HolonomicControl::motionCurve(double totalDistance, double max_accel, double max_vel) {

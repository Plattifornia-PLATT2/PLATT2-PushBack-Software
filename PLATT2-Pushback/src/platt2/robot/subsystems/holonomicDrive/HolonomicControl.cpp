...
void HolonomicControl::moveToPoint(double x_target, double y_target, double target_heading) {
    ...
    std::vector<double> rArray(100,0);
    std::vector<double> wArray(100,0);
    ...
    while (true){
        ...
        motionVector.r = std::clamp(positionPID->calculate(0, p.r), 0.0,1.0);
        motionVector.theta = p.theta;
        motionVector.w = std::clamp(headingPID->calculate(0, angle_error), -1.0, 1.0);
